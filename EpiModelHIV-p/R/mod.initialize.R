...
initialize_msm <- function(x, param, init, control, s) {
  ...
  dat$attr$role.class[which(dat$attr$sex==1)]<-0
  dat$attr$role.class[which(dat$attr$sex==2)]<-1
  ...
  circ <- runif(num) < hiv.circ.prob[race]
  circ[which(sex==2)] <- FALSE
  dat <- set_attr(dat, "circ", circ)
  ...
  dat <- set_attr(dat, "part.art", rep(NA, num))
...
init_status_msm <- function(dat) {
  ...
  status <- dat$attr$diag.status <- rep(0,length(race))
  ...
  max.hiv.time <- vl.aids.onset.int
  ...
  stage.time <- time.infected - min.hiv.time
...
init_sti_msm <- function(dat) {
  ...
  prev.vgc <- get_init(dat, "prev.vgc")
  prev.uct <- get_init(dat, "prev.uct")
  prev.rct <- get_init(dat, "prev.rct")
  prev.vct <- get_init(dat, "prev.vct")

  num <- length(active)
  num.m <- length(which(sex==1))
  num.f <- length(which(sex==2))

  idsUreth <- which(sex==1)
  ...
  # Initialize GC infection at all sites
  idsUGC <- sample(idsUreth, size = round(prev.ugc * num.m), FALSE)
  uGC[idsUGC] <- 1

  idsVGC <- sample(idsVag, size = round(prev.vgc * num.f),FALSE)
  vGC[idsVGC] <- 1

  ###############################################################################
  ########  No MSM so rectal infection will be among females (all vaginal for now)
  ###############################################################################
  # idsRGC <- sample(
  #   setdiff(idsRect, idsVGC),
  #   size = round(prev.rgc * num.f),
  #   FALSE
  # )
  ...
  dat <- set_attr(dat, "vGC.sympt", rep(NA, num))
  ...
  vGC.sympt <- runif(sum(vGC)) < vgc.sympt.prob
  dat <- set_attr(dat, "vGC.sympt", vGC.sympt, posit_ids = as.logical(vGC))

  dat <- set_attr(dat, "rGC.infTime", rep(NA, num))
  dat <- set_attr(dat, "uGC.infTime", rep(NA, num))
  dat <- set_attr(dat, "vGC.infTime", rep(NA, num))
  dat <- set_attr(dat, "rGC.infTime", 1, posit_ids = as.logical(rGC))
  dat <- set_attr(dat, "uGC.infTime", 1, posit_ids = as.logical(uGC))
  dat <- set_attr(dat, "vGC.infTime", 1, posit_ids = as.logical(vGC))
  ...
  dat <- set_attr(dat, "vGC.timesInf", rep(0, num))
  dat <- set_attr(dat, "vGC.timesInf", 1, posit_ids = as.logical(vGC))

  dat <- set_attr(dat, "rGC.tx", rep(NA, num))
  dat <- set_attr(dat, "uGC.tx", rep(NA, num))
  dat <- set_attr(dat, "vGC.tx", rep(NA, num))
  dat <- set_attr(dat, "rGC.tx.prep", rep(NA, num))
  dat <- set_attr(dat, "uGC.tx.prep", rep(NA, num))
  dat <- set_attr(dat, "vGC.tx.prep", rep(NA, num))


  # Initialize CT infection at all sites
  idsUCT <- sample(idsUreth, size = round(prev.uct * num.m), FALSE)
  uCT[idsUCT] <- 1

  idsVCT <- sample(idsVag, size = round(prev.vct * num.f), FALSE)
  vCT[idsVCT] <- 1

  ###############################################################################
  ########  No MSM so rectal infection is among females
  ###############################################################################

  idsRCT <- sample(
    setdiff(idsRect, idsVCT),
    size = round(prev.rct * num.f),
    FALSE
  )
  rCT[idsRCT] <- 1
  ...
  dat <- set_attr(dat, "vCT.sympt", rep(NA, num))
  ...
  vCT.sympt <- runif(sum(vCT)) < vct.sympt.prob
  dat <- set_attr(dat, "vCT.sympt", vCT.sympt, posit_ids = as.logical(vCT))

  dat <- set_attr(dat, "rCT.infTime", rep(NA, num))
  dat <- set_attr(dat, "uCT.infTime", rep(NA, num))
  dat <- set_attr(dat, "vCT.infTime", rep(NA, num))
  dat <- set_attr(dat, "rCT.infTime", 1, posit_ids = as.logical(rCT))
  dat <- set_attr(dat, "uCT.infTime", 1, posit_ids = as.logical(uCT))
  dat <- set_attr(dat, "vCT.infTime", 1, posit_ids = as.logical(vCT))
  ...
  dat <- set_attr(dat, "vCT.timesInf", rep(0, num))
  dat <- set_attr(dat, "vCT.timesInf", 1, posit_ids = as.logical(vCT))

  dat <- set_attr(dat, "rCT.tx", rep(NA, num))
  dat <- set_attr(dat, "uCT.tx", rep(NA, num))
  dat <- set_attr(dat, "vCT.tx", rep(NA, num))
  dat <- set_attr(dat, "rCT.tx.prep", rep(NA, num))
  dat <- set_attr(dat, "uCT.tx.prep", rep(NA, num))
  dat <- set_attr(dat, "vCT.tx.prep", rep(NA, num))

  return(dat)

}

#' @title Initialize the STI status of heterosexual persons in the network
#'
#' @description Sets the initial individual-level disease status of heterosexual persons
#'              in the network, as well as disease-related attributes for
#'              infected persons.
#'
#' @param dat Data object created in initialization module.
#'
#' @export
#' @keywords initiation utility het
#'
init_sti_het <- function(dat) {
...
reinit_msm <- function(x, param, init, control, s) {
  ...
  if (!all(need.for.reinit %in% names(x))) {
    stop("x must contain the following elements for restarting: ",
         "param, control, nwparam, epi, attr, temp, el",
         call. = FALSE)
  }
