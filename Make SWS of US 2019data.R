...
load("/homes/dth2/CAMP 17/NSFG/NSFGsexnet2019_reduced_national.rdata")

#Reset age for 45 year olds to 44 which is the upper limit of the survey
old<-which(NSFGsexnet2019_reduced_national$age>=45)
NSFGsexnet2019_reduced_national <- NSFGsexnet2019_reduced_national[-old,]


#Split out the data to males and females
m <- which(NSFGsexnet2019_reduced_national$sex == 1)
f <- which(NSFGsexnet2019_reduced_national$sex == 2)

males <- NSFGsexnet2019_reduced_national[m,]
females <- NSFGsexnet2019_reduced_national[f,]

##ADD role class
males$role.class <- rep(1,length(males$age))
females$role.class <- rep(2,length(females$age))

## we want to limit the pop to 15-45 year-olds.
...
save(het_national,file = "het_national_2019.rda")
...
#ACT RATE WILL COME FROM DAVIDS DATA.
...
#There are missing values for 361 partner race and 20225 partner age variable
...
##PULL in 2011 data for age imputation.
load("~/CAMP 17/NSFG/data/l_het_2011.rda")

for(i in 15:45){

  bm.m <- which(alters$race3 == 1 & alters$sex == 1 & alters$age == i & is.na(alters$p_age) == TRUE)
  bm.k <- which(l_het$race3 == 1 & l_het$sex == 1 & l_het$age == i & is.na(l_het$p_age) == FALSE)
  j <- sample(l_het$p_age[bm.k],length(bm.m),replace=TRUE)
  alters$p_age[bm.m]<-j

  hm.m <- which(alters$race3 == 2 & alters$sex == 1 & alters$age == i & is.na(alters$p_age) == TRUE)
  hm.k <- which(l_het$race3 == 2 & l_het$sex == 1 & l_het$age == i & is.na(l_het$p_age) == FALSE)
  j <- sample(l_het$p_age[hm.k],length(hm.m),replace=TRUE)
  alters$p_age[hm.m]<-j

  om.m <- which(alters$race3 == 3 & alters$sex == 1 & alters$age == i & is.na(alters$p_age) == TRUE)
  om.k <- which(l_het$race3 == 3 & l_het$sex == 1 & l_het$age == i & is.na(l_het$p_age) == FALSE)
  j <- sample(l_het$p_age[om.k],length(om.m),replace=TRUE)
  alters$p_age[om.m]<-j

  bf.m <- which(alters$race3 == 1 & alters$sex == 2 & alters$age == i & is.na(alters$p_age) == TRUE)
  bf.k <- which(l_het$race3 == 1 & l_het$sex == 2 & l_het$age == i & is.na(l_het$p_age) == FALSE)
  j <- sample(l_het$p_age[bf.k],length(bf.m),replace=TRUE)
  alters$p_age[bf.m]<-j

  hf.m <- which(alters$race3 == 2 & alters$sex == 2 & alters$age == i & is.na(alters$p_age) == TRUE)
  hf.k <- which(l_het$race3 == 2 & l_het$sex == 2 & l_het$age == i & is.na(l_het$p_age) == FALSE)
  j <- sample(l_het$p_age[hf.k],length(hf.m),replace=TRUE)
  alters$p_age[hf.m]<-j

  of.m <- which(alters$race3 == 3 & alters$sex == 2 & alters$age == i & is.na(alters$p_age) == TRUE)
  of.k <- which(l_het$race3 == 3 & l_het$sex == 2 & l_het$age == i & is.na(l_het$p_age) == FALSE)
  j <- sample(l_het$p_age[of.k],length(of.m),replace=TRUE)
  alters$p_age[of.m]<-j
...
save(het_national,file = "het_national_2019_clean.rda")
...
save(d_het,file = "/homes/dth2/CAMP 17/NSFG/data/d_het_2019.rda")
save(l_het,file = "/homes/dth2/CAMP 17/NSFG/data/l_het_2019.rda")
